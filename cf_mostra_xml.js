console.log('Sintaxe: cf_mostra_xml(xml,elemento_id,pixels_identa,cor_node,cor_menos,cor_mais,cor_string,cor_inteiro,cor_float,cor_attributo,cor_atributo_valor); /*depende de jquery*/');
function cf_mostra_xml(xml_original,el,identa,cornode,cormenos,cormais,cor_string,cor_inteiro,cor_float,coratributo,coratributovalor) {
    ...
    function mnode(node) {
        ...
        for (yy = 0; yy < xx.length; yy++) {
            ...
            if(xx[yy].childNodes[0] && xx[yy].childNodes[0].hasChildNodes()) {
                mhtml += '<span data_valor="'+nodecont+'" class="cf_xml_reader_aberto cf_xml_reader_node_abre_fecha">&lt;' + xx[yy].nodeName;
            } else {
            ...
            mhtml+='&gt;</span>';
            if(xx[yy].childNodes[0] && xx[yy].childNodes[0].hasChildNodes()) {
                mhtml += '<div class="cf_xml_reader_node_conteudo" data_valor="'+nodecont+'">';
            }
    ...
    var xml=xml_original.documentElement;
    var nodecont=1;
